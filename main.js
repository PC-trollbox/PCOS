...
if (!localStorage.getItem("dbpwd")) {
	...
	document.body.innerHTML = "<b>Pre-boot database password entry</b><br>Enter database password: ";
	var string = "";
	onkeypress = function(e) {
		if (e.key == "Enter") {
			document.body.innerHTML = "Saving...";
			localStorage.setItem("dbpwd", string);
			location.reload();
		} else {
			string = string + e.key;
	...
	throw new Error("Database password not found on local machine!");
}
db.password = localStorage.getItem("dbpwd");
try {
	// browser detect
	var BrowserDetect = {
		init: function() {
